...
const NAV_ITEMS = [
    ...
    { id: "research", label: "Research Papers", icon: Tags, color: "text-cyan-500" },
    { id: "case-studies", label: "Case Studies", icon: FileText, color: "text-purple-500" },
    { id: "help", label: "Help", icon: HelpCircle, color: "text-amber-500" },
];

export default function Sidebar({ activeTab, onTabChange, onLogout, courses, topics }) {
    ...
    return (
        <div className="bg-[#FFF7ED]">
            ...
            <aside
                className={`
                    fixed inset-y-4 left-0 z-50 flex flex-col overflow-hidden rounded-[28px]
                    border border-white/10 bg-[#79A68E] shadow-xl transition-all duration-300
            ...
            >
                {/* ===== HEADER ===== */}
                <div
                    className={`flex pb-4 pt-5 ${isCollapsed
                        ? "flex-col items-center gap-3 px-2"
                        : "items-center justify-between gap-2 px-4"
                        }`}
                >
                    {!isCollapsed ? (
                        <Link href="/admin" className="flex min-w-0 items-center gap-2.5" onClick={() => onTabChange("dashboard")}>
                            ...
                            <span className="truncate text-base font-extrabold tracking-tight text-white">
                                Admin<span className="text-brand-light">.</span>
                            </span>
                        </Link>
                    ) : (
                        <Link
                ...
                <nav className="flex-1 space-y-1 overflow-y-auto px-3">
                    <Link
                        href="/admin"
                        className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-bold transition-colors ${activeTab === "dashboard"
                    ...
                    >
                        <BarChart3
                            className={`h-5 w-5 shrink-0 ${activeTab === "dashboard" ? "text-white" : "text-white/50"}`}
                            strokeWidth={2}
                        />
                    ...
                    {NAV_ITEMS.map((item) => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-bold transition-colors ${isActive
                                    ? "bg-white/20 text-white"
                                    : "text-white/70 hover:bg-white/10 hover:text-white"
                                    } ${isCollapsed ? "justify-center" : ""}`}
                            >
                                <Icon
                                    className={`h-5 w-5 shrink-0 ${isActive ? "text-white" : "text-white/50"}`}
                                    strokeWidth={2}
                                />
