// app/components/adminPanelComp/AgendaFullView.jsx
"use client";
...
// ===== THEME TOKENS (kept in sync with DashboardPreview) =====
...
// Time-table window. Widen HOUR_WIDTH_DESKTOP / the range to show more of the day.
const START_HOUR = 7;
const END_HOUR = 13;
const HOUR_WIDTH_DESKTOP = 176; // px per hour column on wider screens
const HOUR_WIDTH_MOBILE = 128; // px per hour column on narrow screens
const CARD_MIN_RATIO = 0.8; // min event-card width, as a fraction of one hour column
...
// Tracks a simple "is this a narrow viewport" flag so the time-table can use
// a tighter hour column on phones without needing a layout-shifting redraw.
...
// Sample events, anchored to "today" + the next couple of days so the demo
// always has something to show regardless of when it's viewed.
    ...
    return useMemo(() => {
        ...
        const events = {
            [toKey(today)]: [
                {
                    id: "e1",
                    title: "Learning About Hick's Law - Choice and Limitation",
                    subtitle: "UX Design Foundations · Course",
                    startHour: 7,
                    startMin: 0,
                    endHour: 8,
                    endMin: 30,
                ...
                {
                    id: "e2",
                    title: "Attend the Webinar on Time",
                    subtitle: "Webinar UX Design in Digital Era · Event",
                    startHour: 8,
                    startMin: 0,
                    endHour: 10,
                    endMin: 0,
                ...
                {
                    id: "e3",
                    title: "Complete the Last Challenge",
                    subtitle: "Challenge Designer Tool Kit · Challenge",
                    startHour: 9,
                    startMin: 30,
                    endHour: 11,
                    endMin: 0,
            ...
            [toKey(tomorrow)]: [
                {
                    id: "e4",
                    title: "Common Design Patterns Review",
                    subtitle: "Common Design Patterns · Course",
                    startHour: 9,
                    startMin: 0,
                    endHour: 10,
                    endMin: 0,
...
export default function AgendaFullView({ onBack }) {
    const { events, today } = useSampleEvents();
                                ...
                                {cells.map((cell, i) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`relative mx-auto flex aspect-square w-full max-w-[32px] items-center justify-center rounded-full text-xs font-semibold transition-colors
                        ...
                        <>
                                        ...
                                        {dayEvents.map((ev, idx) => {
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    style={{
                                                        left,
                                                        width: Math.max(width, cardMinWidth),
                                                        top,
                                                        background: `${ev.tone}1f`,
                                                    }}
                                                >
                                                    <div className="mb-2 flex items-center justify-between gap-2">
                                                        <span
                                                            className="truncate rounded-full bg-white/70 px-2 py-0.5 text-[10px] font-bold"
                                                            style={{ color: ev.tone }}
                                                        >
                            ...
                            {/* <div className="mt-3 h-1 w-full overflow-hidden  rounded-full bg-gray-100">
                                <div
                                    ...
                                    style={{ width: "35%", marginLeft: `${scrollPct * 65}%`, background: LEARNING }}
                                />
                            </div> */}
                        </>
                    )}
