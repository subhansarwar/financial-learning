// app/components/userDashboardComp/userCoursesComp/StepTwoCurriculum.jsx
"use client";
...
function LectureVideoDrop({ lecture, onFile }) {
    ...
    return (
        <div
            ...
            className="flex min-h-[110px] cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-[#365B50]/40 bg-[#365B50]/10 p-4 text-center transition-colors hover:bg-[#365B50]/20"
        >
            <input
                ref={inputRef}
                type="file"
            ...
            />
            <Video className="mb-2 h-6 w-6 text-[#365B50]" strokeWidth={1.5} />
            <p className="text-xs font-bold text-[#365B50]">
...
export default function StepTwoCurriculum({ sections, onChange }) {
    ...
    return (
        <div className="space-y-5">
                ...
                <div key={section.id} className="rounded-xl2 border border-line bg-cream-2/30 p-4 sm:p-5">
                    <div className="mb-4 flex items-center justify-between gap-3">
                        <div className="flex flex-1 items-center gap-2">
                            <Layers3 className="h-4 w-4 shrink-0 text-[#365B50]" strokeWidth={2} />
                    ...
                    <button
                        ...
                        className="mt-3 inline-flex items-center gap-1.5 text-sm font-bold text-[#365B50] hover:underline"
                    >
            ...
            <button
                onClick={addSection}
                className="inline-flex items-center gap-1.5 text-sm font-bold text-[#365B50] hover:underline"
            >
                <Plus className="h-4 w-4" strokeWidth={2.5} />
                Add Curriculum Section
            </button>

            {(hasEmptySectionName || !hasValidLecture) && (
                <div className="mt-4 rounded-lg bg-amber-50 p-3 border border-amber-200">
                    <p className="text-sm text-amber-700">
                        <span className="font-bold">⚠️ Required:</span>
                        {hasEmptySectionName && " All section names must be filled."}
                        {!hasValidLecture && " At least one lecture name must be filled in each section."}
                    </p>
                </div>
            )}
        </div>
