...
from app.crud.users.user_api import update_profile, set_avatar_url, set_password
...
async def change_password(payload: ChangePasswordRequest, db: SessionDep, current_user: CurrentUser) -> MessageResponse:
    ...
    await set_password(db, current_user, payload.new_password)
...
async def upload_avatar(db: SessionDep, current_user: CurrentUser, file: UploadFile = File(...)) -> AvatarUploadResponse:
    ...
    await set_avatar_url(db, current_user, avatar_url)
...
#     await set_email(db, current_user, new_email)
#     return UserRead.model_validate(current_user)
