...
from app.services.courses import progress_service

router = APIRouter()


async def _build_course_detail(db: AsyncSession, course: Course) -> CourseDetail:
    modules = await module_crud.list_for_course(db, course.id)
    module_reads = []
    for m in modules:
        lessons = await lesson_crud.list_for_module(db, m.id)
        module_reads.append(
            ModuleRead(
                id=m.id,
...
async def list_courses(
    ...
    courses, _ = await course_crud.list_courses(
...
async def list_my_enrollments(db: SessionDep, current_user: CurrentUser) -> list[EnrollmentRead]:
    enrollments = await enrollment_crud.list_for_user(db, current_user.id)
...
async def enroll(course_id: uuid.UUID, db: SessionDep, current_user: CurrentUser) -> EnrollmentRead:
    ...
    enrollment = await enrollment_crud.get_or_create(db, user_id=current_user.id, course_id=course_id)
...
async def get_progress(course_id: uuid.UUID, db: SessionDep, current_user: CurrentUser) -> CourseProgressResponse:
    ...
    return await progress_service.get_progress(db, user_id=current_user.id, course_id=course_id)
...
async def complete_lesson(lesson_id: uuid.UUID, db: SessionDep, current_user: CurrentUser) -> CourseProgressResponse:
    ...
    try:
        return await progress_service.complete_lesson(db, user=current_user, lesson_id=lesson_id)
    except progress_service.CourseNotFoundForLessonError:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Lesson not found")
