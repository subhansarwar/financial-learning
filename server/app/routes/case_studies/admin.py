...
async def create_case_study(payload: CaseStudyCreate, db: SessionDep) -> CaseStudyRead: # , admin: CurrentAdmin
    ...
    case_study = await case_study_crud.create_case_study(db, created_by=None, **payload.model_dump())
...
async def delete_case_study(case_study_id: uuid.UUID, db: SessionDep) -> MessageResponse:  # _admin: CurrentAdmin
    case_study = await case_study_crud.get_by_id(db, case_study_id)
    if case_study is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Case study not found")
